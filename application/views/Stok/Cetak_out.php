      ...
      <table class="table table-hover table-bordered">
        ...
        <tbody>
          <?php foreach ($data_header as $dh) : ?>
            <?php $no = 1;
            $detail = $this->db->query("SELECT *, (SELECT nama FROM barang WHERE kode_barang=out_detail.kode_barang) as nama_barang FROM out_detail WHERE invoice_keluar = '$dh->invoice_keluar' AND kode_barang IN (SELECT kode_barang FROM barang) ORDER BY invoice_keluar ASC")->result();
            foreach ($detail as $s) :
            ?>
              <tr>
                <td><?= $no++; ?></td>
            ...
            <?php endforeach; ?>
            <tr>
              <td class="table-dark" colspan="6"><?= $dh->invoice_keluar; ?> <span class="float-right"><b>TOTAL</b></span></td>
